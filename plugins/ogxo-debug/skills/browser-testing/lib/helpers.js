...
async function launchBrowser(browserType = 'chromium', options = {}) {
  const defaultOptions = {
    ...
    // Chromium's sandbox stays on unless explicitly disabled (needed only for root-in-container runs)
 ...
 * Defaults to the 'load' state; pass options.waitForSelector to wait for the
 * element you actually need. 'networkidle' is still accepted via
 * options.waitUntil but is DISCOURAGED by Playwright (use only for one-off
 * reconnaissance screenshots, never in tests).
...
async function takeScreenshot(page, name, options = {}) {
  ...
  const filename = path.join(os.tmpdir(), `${name}-${timestamp}.png`);

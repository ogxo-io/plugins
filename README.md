...
| `ogxo-statusline` | Status line for Claude Code and Grok Build: model, context use, git branch and changes, session time, effort, output tokens, and session cost. Claude Code also shows thinking, prompt-cache state, and 5-hour / 7-day usage bars. Grok Build uses the live context window, session output and cache-read share, and the active turn. `/ogxo-statusline:setup` installs the Claude line; `/ogxo-statusline:setup-grok` installs the Grok line. Requires `jq`. | 0.2.1 |
